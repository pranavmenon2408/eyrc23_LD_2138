...
class swift():
    ...
    def __init__(self):
        ...
        self.search_setpoints=[[-8,9,30],[-8,-8,30],[-6,-6,30],[8,-6,30],[5,-5,30],[6,8,30],[2,2,30],[11,11,35],[11,11,37]]
    ...
    def pid(self):
        ...
        if self.disarm_flag==1:
            ...
            if fabs(self.align_x) > 0.0 and fabs(self.align_y)>0.0:
                if self.wait==0 and self.stop_detect==0:
                    if fabs(self.align_x)<20 and fabs(self.align_y)<20 and fabs(self.drone_position[2]-30)<=0.20:

                        
                            self.wc.x=round(self.drone_position[0],1)
                            self.wc.y=round(self.drone_position[1],1)
                            self.wc.z=round(self.drone_position[2],1)
                    ...
                    #alien_detected=1
                        ...
                        self.setpoint.append([self.drone_position[0]+self.align_x/150,self.drone_position[1]+self.align_y/150,30])
            ...
            if (self.alien_flag==1):
                if(abs(self.drone_position[0]-self.wc.x)>3.0 or abs(self.drone_position[1]-self.wc.y)>3.0):
                    self.alien_flag=0
                    self.wait=0
                else:
                    self.wait=1
            ...
            if((0<abs(self.prev_error[0])<=self.threshold) and (0<abs(self.prev_error[1])<=self.threshold) and (0<abs(self.prev_error[2])<=self.threshold)):
                ...
                if len(self.setpoint)==1 or (self.alien_flag==1):
                    ...
                    if self.ind<len(self.search_setpoints):
                        ...
                        self.ind+=1
                        self.index=0
                        if self.ind==(len(self.search_setpoints)-1):
                            self.stop_detect=1
                        #self.alien_flag=0
